...
NEW_LOTO7 用の軽量・再開可能パイプライン。

目的:
  1. scrapingloto7.py を任意で先に実行し、loto7.csv を最新化する
  2. 未来リークなしの walk-forward バックテストを行う
  3. 途中中断しても resume_state.json と結果CSVから再開する
  4. GitHub Actions で長時間実行しても、一定件数ごとに commit/push できる
  5. 購入口数はデフォルト5口
...
def score_numbers(train: Sequence[Draw]) -> Dict[int, float]:
    """頻度・直近性・休眠のバランススコア。未来データは受け取らない。"""
...
def generate_candidates(train: Sequence[Draw], purchase_count: int = 5, pool_size: int = 18) -> List[Tuple[int, ...]]:
    if not train:
...
def git_commit_push(message: str, paths: Sequence[str], retries: int = 3) -> bool:
    if os.environ.get("DISABLE_GIT_PUSH", "").lower() in {"1", "true", "yes"}:
        print("[GIT] DISABLE_GIT_PUSH is set. skip commit/push.")
        return False
    if not Path(".git").exists():
        print("[GIT] .git not found. skip commit/push.")
        return False

    existing_paths = [p for p in paths if Path(p).exists()]
    if not existing_paths:
        print("[GIT] no existing paths to add. skip.")
        return False
...
def run_scraping(csv_path: str, months: int) -> None:
...
def run_backtest(
    ...
    print(f"[BACKTEST] targets={len(targets)} last_completed={last_completed} purchase_count={purchase_count}")
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="NEW_LOTO7 resumable pipeline")
    parser.add_argument("--csv", default="loto7.csv")
    parser.add_argument("--output-dir", default="outputs")
    parser.add_argument("--resume-state", default="outputs/resume_state.json")
    parser.add_argument("--run-scraping", action="store_true", help="先に scrapingloto7.py でCSV更新する")
    ...
    if args.push_final:
        ...
        git_commit_push("Update LOTO7 pipeline outputs", paths)
