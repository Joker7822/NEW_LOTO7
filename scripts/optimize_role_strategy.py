...
方針:
  ...
  - 5口合計になるように配分
  - main_bestの過集中を制限し、古い高配当に引っ張られすぎないようにする
...
def normalize_counts(scores: Dict[str, float], purchase_count: int, max_main_best: int) -> Dict[str, int]:
    ...
    for _ in range(purchase_count):
        ...
        candidates = [role for role in candidates if counts[role] < 2] or candidates or roles
        chosen = max(candidates, key=lambda r: shifted.get(r, 0.0) / float(counts[r] + 1))
        counts[chosen] += 1

    # 本命か高一致を最低1口残す。
    if counts.get("main_best", 0) + counts.get("high_match", 0) <= 0:
        weakest = min([r for r in roles if counts[r] > 0], key=lambda r: shifted.get(r, 0.0))
        counts[weakest] -= 1
        anchor = "high_match" if shifted.get("high_match", 0.0) >= shifted.get("main_best", 0.0) else "main_best"
        if anchor == "main_best" and max_main_best <= 0:
            anchor = "high_match"
        counts[anchor] += 1
    ...
    while sum(counts.values()) > purchase_count:
        ...
        counts[role] -= 1
    while sum(counts.values()) < purchase_count:
        candidates = [role for role in roles if not (role == "main_best" and counts[role] >= max_main_best)] or roles
        role = max(candidates, key=lambda r: shifted.get(r, 0.0) / float(counts[r] + 1))
        counts[role] += 1
    return counts


def build_strategy(summary: Dict[str, object], purchase_count: int, min_completed_draws: int, max_main_best: int) -> Dict[str, object]:
    ...
    if completed < min_completed_draws or not roles:
        ...
        if max_main_best < counts.get("main_best", 0):
            counts["main_best"] = max(0, max_main_best)
            while sum(counts.values()) < purchase_count:
                counts["recent120"] = counts.get("recent120", 0) + 1
        ...
        counts = normalize_counts(scores, purchase_count, max_main_best)
        reason = "optimized_from_role_ensemble_backtest_with_main_best_cap"
...
def main() -> int:
    ...
    if args.max_main_best < 0:
        raise SystemExit("--max-main-best must be zero or positive")
    if not Path(args.summary).exists():
        fallback = build_strategy({}, args.purchase_count, args.min_completed_draws, args.max_main_best)
    ...
    strategy = build_strategy(summary, args.purchase_count, args.min_completed_draws, args.max_main_best)
