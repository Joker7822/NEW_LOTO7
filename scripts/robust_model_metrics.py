...
The normal payout ROI can be dominated by one unusually large prize.  This
module evaluates the same chronological predictions while also reporting:

* ROI after removing the largest one and two draw payouts.
* Payout concentration and HHI.
* Median and worst calendar-year ROI.
* Downside CVaR of draw-level profit.
* A deterministic bootstrap interval for payout ROI.

These are historical stability diagnostics, not winning probabilities.
"""
...
from loto7_evolution_trainer import Draw, Genome, evaluate_ticket, generate_tickets, genome_from_dict, load_draws  # noqa: E402
...
def indices_for_years(draws: Sequence[Draw], base_indices: Sequence[int], start_year: Optional[int], end_year: Optional[int]) -> List[int]:
    selected: List[int] = []
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build robust payout-concentration metrics for a LOTO7 model.")
    ...
    base_indices = select_target_indices(draws, min_train_draws=1, holdout_start_draw=2, holdout_end_draw=None)
