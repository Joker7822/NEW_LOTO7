...
目的:
  - 最新予測、履歴照合、holdout、recent era、自己進化、Role/Regime系を1フォルダで確認できるようにする
...
出力:
  ...
  outputs/txt_reports/04_recent_era_report.txt
  outputs/txt_reports/05_model_selection_report.txt
  outputs/txt_reports/06_model_self_evolution_standalone.txt
  outputs/txt_reports/07_model_self_evolution_integrated.txt
  outputs/txt_reports/08_role_ensemble_backtest.txt
  outputs/txt_reports/09_role_strategy_optimizer.txt
  outputs/txt_reports/10_regime_strategy.txt  # source exists only
  outputs/txt_reports/11_progress_summary.md
  outputs/txt_reports/12_json_summary_digest.txt
...
REPORTS = [
    ...
    {
        "order": "04",
        "title": "Recent Era Holdout Report",
        "source": "outputs/holdout/recent_era_report.txt",
        "target": "04_recent_era_report.txt",
        "description": "2020年以降など直近時代のholdout成績。古い大当たり偏重を確認するための評価。",
    },
    {
        "order": "05",
        "title": "Model Selection Report",
        "source": "outputs/holdout/model_selection_report.txt",
        "target": "05_model_selection_report.txt",
    ...
    },
    {
        ...
        "target": "06_model_self_evolution_standalone.txt",
    ...
    },
    {
        ...
        "target": "07_model_self_evolution_integrated.txt",
    ...
    },
    {
        ...
        "target": "08_role_ensemble_backtest.txt",
    ...
    },
    {
        ...
        "target": "09_role_strategy_optimizer.txt",
...
def write_json_digest(out_dir: Path, created_at: str) -> List[Dict[str, object]]:
    ...
    for item in items:
        ...
        lines.append("")
    write_text(out_dir / "12_json_summary_digest.txt", "\n".join(lines).rstrip() + "\n")
    return items
...
def write_index(out_dir: Path, created_at: str, entries: Sequence[Dict[str, object]], json_items: Sequence[Dict[str, object]]) -> None:
    ...
    lines.extend(
        [
            ...
            "2. 04_recent_era_report.txt",
            "3. 10_regime_strategy.txt / generated only after regime report exists",
            "4. 09_role_strategy_optimizer.txt",
            "5. 08_role_ensemble_backtest.txt",
            "6. 03_holdout_report.txt",
            "7. 06_model_self_evolution_standalone.txt",
            "8. 02_prediction_history_result.txt",
            "",
            "[Combined]",
...
def write_combined(out_dir: Path, entries: Sequence[Dict[str, object]], created_at: str) -> None:
    ...
    json_digest = out_dir / "12_json_summary_digest.txt"
    if json_digest.exists():
        parts.append("\n" + "#" * 80)
        parts.append("# 12 JSON Summary Digest")
        parts.append("#" * 80)
        parts.append("")
