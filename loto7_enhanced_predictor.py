...
NEW_LOTO7 向けの精度改善版予測器。

主な追加点:
    - 時系列減衰つき番号スコア
    - 時系列減衰つきペア・トリプル相関スコア
    - 既存バックテスト明細から高一致パターンを再利用
    - 未来リーク防止のため、バックテスト時は検証対象日より前の高一致履歴だけ使用
    - 既存 loto7_logic_predictor.py を壊さない追加実装

注意:
    ロト7は独立抽せんです。的中保証は確認できません。
    本スクリプトは候補生成ロジックの改善版です。
"""
...
def load_hit_patterns(
    ...
    with csv_path.open("r", encoding="utf-8-sig", newline="") as f:
        ...
        for row in reader:
            ...
            for i in range(1, 51):
                pred_key = f"予測{i}"
                match_key = f"予測{i}_本数字一致"
                if pred_key not in row:
                    break
                try:
                    matches = int(str(row.get(match_key, "0") or "0"))
                except ValueError:
                    matches = 0
                if matches >= min_matches:
                    ticket = parse_ticket(row.get(pred_key, ""))
                    if ticket:
...
def pattern_similarity(ticket: Sequence[int], patterns: Sequence[Sequence[int]]) -> float:
    ...
    for idx, pattern in enumerate(patterns):
        ...
        # 4個以上一致した過去候補に近い構造を加点。ただし丸写しは軽く抑制。
        if overlap >= 6:
            sim = 0.35
        elif overlap == 5:
            sim = 0.75
...
def enhanced_predict(
    ...
    if len(draws) < 2:
        raise ValueError("予測には2回以上の抽せんデータが必要です。")
...
def prediction_csv_header(save_count: int) -> List[str]:
    header = ["抽せん日"]
...
def save_latest_txt(path: str, target_date: str, tickets: Sequence[TicketScore]) -> None:
    ...
    for i, ticket in enumerate(tickets, start=1):
        ...
        lines.append(
            ...
            f" / スコア正規化値: {scores[i - 1]:.3f}".rstrip("0").rstrip(".")
            + f" / 戦略: {ticket.strategy}"
            + f" / pair:{detail.get('pair', 0):.3f}"
            + f" / triple:{detail.get('triple', 0):.3f}"
            + f" / pattern:{detail.get('pattern', 0):.3f}"
        )
...
def enhanced_backtest(
    ...
    for i in range(start, len(draws)):
        ...
        results = [
            classify_loto7_prize(t.ticket, actual.main, actual.bonus, DEFAULT_PRIZE_TABLE)
            for t in tickets
        ]
        ...
        draw_purchase = len(tickets) * DEFAULT_UNIT_COST
        total_purchase += draw_purchase
        total_prize += draw_prize

        grades = [r.grade for r in results if r.grade is not None]
        for r in results:
            grade_dist["ハズレ" if r.grade is None else f"{r.grade}等"] += 1
        best_grade = min(grades) if grades else None
        best_grade_dist["ハズレ" if best_grade is None else f"{best_grade}等"] += 1
        ...
        row: Dict[str, object] = {
            ...
            "最高等級": "ハズレ" if best_grade is None else f"{best_grade}等",
            "購入金額": draw_purchase,
            "当せん金額": draw_prize,
            "収支": draw_prize - draw_purchase,
        }
        for idx, (ticket, result) in enumerate(zip(tickets, results), start=1):
            ...
            row[f"予測{idx}_等級"] = "ハズレ" if result.grade is None else f"{result.grade}等"
    ...
    summary = {
        ...
        "1回あたり口数": num_tickets,
        "候補プール": pool_size,
        "1口目平均一致数": round(sum(top1_hits) / len(top1_hits), 6) if top1_hits else 0,
        "全口ベスト平均一致数": round(sum(best_hits) / len(best_hits), 6) if best_hits else 0,
        "1口目_3個以上率": round(rate(top1_hits, 3), 6),
        "1口目_4個以上率": round(rate(top1_hits, 4), 6),
        "全口ベスト_3個以上率": round(rate(best_hits, 3), 6),
        "全口ベスト_4個以上率": round(rate(best_hits, 4), 6),
        "総購入金額": total_purchase,
        "総当せん金額": total_prize,
        "総収支": total_prize - total_purchase,
        "総回収率": round(total_prize / total_purchase, 6) if total_purchase else 0,
        "全予測口等級分布": dict(sorted(grade_dist.items())),
        "各回ベスト等級分布": dict(sorted(best_grade_dist.items())),
    }
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Loto7 精度改善版予測・バックテスト")
    ...
    if not draws:
        raise SystemExit("抽せんデータを読み込めませんでした。")

    target_date = next_friday_after(draws[-1].date)
    tickets = enhanced_predict(
        draws,
    ...
    print("注意: スコア正規化値は当選確率ではありません。的中保証は確認できません。")
    return 0
